...
# Verifica Wan 2.2
WAN_STATUS=$(python3 -c "import yaml; print(yaml.safe_load(open('configs/models.yaml')).get('video', {}).get('wan_2_2_5b', {}).get('status', 'not_installed'))")
...
# Verifica LTX Video
LTX_STATUS=$(python3 -c "import yaml; print(yaml.safe_load(open('configs/models.yaml')).get('video', {}).get('ltx_video', {}).get('status', 'not_installed'))")
