...
if [ ! -f "$WAN_2_2_14B_BASE_DIR/model_index.json" ] || [ ! -f "$WAN_2_2_14B_BASE_DIR/transformer/config.json" ]; then
    ...
    echo "Downloading Wan2.2-I2V-14B-Diffusers base model (full)..."
    $PYTHON_BIN -c "from huggingface_hub import snapshot_download; snapshot_download(repo_id='Wan-AI/Wan2.2-I2V-14B-Diffusers', local_dir='$WAN_2_2_14B_BASE_DIR', allow_patterns=['*.json', '*.txt', 'vae/**', 'scheduler/**', 'tokenizer/**', 'image_encoder/**'])"
else
