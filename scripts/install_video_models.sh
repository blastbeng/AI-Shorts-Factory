...
if [ ! -f "$WAN_BASE_DIR/model_index.json" ] || [ ! -f "$WAN_BASE_DIR/transformer/config.json" ]; then
    ...
    echo "Downloading Wan2.2-TI2V-5B-Diffusers base model (full)..."
    $PYTHON_BIN -c "from huggingface_hub import snapshot_download; snapshot_download(repo_id='Wan-AI/Wan2.2-TI2V-5B-Diffusers', local_dir='$WAN_BASE_DIR')"
else
