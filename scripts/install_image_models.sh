...
FLUX_STATUS=$(python3 -c "import yaml; print(yaml.safe_load(open('configs/models.yaml')).get('image', {}).get('flux', {}).get('status', 'not_installed'))")
...
QWEN_STATUS=$(python3 -c "import yaml; print(yaml.safe_load(open('configs/models.yaml')).get('image', {}).get('qwen_image', {}).get('status', 'not_installed'))")
